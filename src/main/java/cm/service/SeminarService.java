...
//    }
    ...
    public AttendanceListVO getAttendanceList(Long klassSeminarId) {
        ...
        List<Attendance> attendances=attendanceDAO.listByKlassSeminarId(klassSeminarId);
    ...
    public Long getPresentatingTeamId(Long klassSeminarId) {
        List<Attendance> attendances=attendanceDAO.listByKlassSeminarId(klassSeminarId);
        for(int i=0;i<attendances.size();i++)
            if(attendances.get(i).getIsPresent().equals(1))
            {
                return attendanceDAO.getTeamIdByAttendanceId(attendances.get(i).getId());
            }
        return attendances.get(0).getId();
    }

    public SeminarInfoVO getSeminarInfoING() {
        List<Round> rounds=roundDAO.listByCourseId(course.getId());
    ...
    public void scoreReport(List<BigDecimal> score) {
        List<Attendance> attendances=attendanceDAO.listByKlassSeminarId(klassSeminar.getId());
        for(int i=0;i<attendances.size();i++) {
            //获取展示的所有小组
            Team team=teamDAO.findByAttendanceId(attendances.get(i).getId());
            //得到所有当前分数
